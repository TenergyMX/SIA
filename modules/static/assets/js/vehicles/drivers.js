    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $(".card-img img")
                ...
                .on("error", function () {
                    $(this).css("padding", "100px"); // Aplica padding si la imagen no se carga
                })
                .each(function () {
                    if (!this.src || this.src.trim() === "") {
                        $(this).css("padding", "100px"); // Aplica padding si la imagen está vacía
                    } else {
                        $(this).css("padding", "0px"); // Asegura que no tenga padding si hay imagen
                    }
                });
        },
        ...
        columns: [
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (row.status_licence) {
                        ...
                        if (data) {
                            // Show the bell icon if alert is active (close to expiration)
                            $("#v-licence .alert-icon").html(
                                '<i class="fa-regular fa-circle-exclamation" style="color:rgb(255, 174, 0);"></i>'
                            );
                            ...
                            // If the first license is not near expiration, show nothing
                            $("#v-licence .alert-icon").html("");
                            return '<i class="fa-solid fa-check" style="color:green"></i>';
                        }
                    } else {
                        // For subsequent licenses, show the cross icon
                        return '<i class="fa-solid fa-xmark" style="color:rgb(255, 0, 0)"></i>';
                    }
                },
            },
            { title: "ID", data: "id" },
...
function get_vehicles(selectedVehicleId) {
    $.ajax({
        ...
        success: function (response) {
            var select = $("#vehicle");
            select.html("<option value='' disabled selected>Seleccione un vehículo</option>");
            $.each(response.data, function (index, vehicle) {
                var selected = vehicle.id == selectedVehicleId ? "selected" : "";
                select.append(`<option value="${vehicle.id}" ${selected}>${vehicle.name}</option>`);
            });
        },
        error: function (xhr, status, error) {
