    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            $("#equipments_tools_table").DataTable().ajax.reload(null, false);
                        });
                    } else {
                        Swal.fire({
                ...
                error: function (xhr, status, error) {
                    console.error("Error al eliminar la categoría:", xhr.responseJSON);

                    let mensaje = "Hubo un error al eliminar la categoría.";
...
function clear_category_form() {
    const form = $("#form_add_category_equip")[0];
    form.reset();
    $('#form_add_category_equip [name="id"]').val("");
    $("#active-field").addClass("d-none");
    $("#mdl-crud-equipments-tools-category .modal-title").text("Agregar Categoría");
    $("#form_add_category_equip").attr("onsubmit", "add_equipment_category(); return false");
}
